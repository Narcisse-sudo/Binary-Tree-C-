...
class Data{
public :
    map<string, vector<string>> cat;//Modalités uniques
    ...
    Data(string texte,string TVar[],int indiceVarY);//Charge les données depuis un fichier CSV
    friend ostream & operator<<(ostream & os,Data D);//Affiche les données
};
